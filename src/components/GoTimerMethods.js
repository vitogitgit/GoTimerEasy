...
import { elseStyle } from './GoTimerStyle';
...
export const onCountdown = (isBlack, component) => {
  ...
  const srcBlack = Sounds.referee.REF_BLACK_COUNTDOWN_START;
  const srcWhite = Sounds.referee.REF_WHITE_COUNTDOWN_START;
...
export const runnerColor = (isBlack, component) => {
  ...
  if (timerStart && isBlack === trunBlack) {
    return elseStyle.RUNNER_BORDER_COLOR;
  }
  return elseStyle.STOPPER_BORDER_COLOR;
};
...
export const playStatusTrunOpponent = (isBlack, component) => (
  component.state.timerStart && isBlack !== component.state.trunBlack
);

export const playVideoFromFisrtStart = (component) => {
  const { REF_BLACK_START, REF_WHITE_START } = Sounds.referee;
